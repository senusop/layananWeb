				...
				<div class="content">
					<div class="block">
						<div class="block-header">
							<?php echo anchor('kelas_wali/tambah','<i class="fa fa-plus"></i> Tambah','class=" btn btn-sm btn-success" data-toggle="tooltip" title="Tambah kelas"');?>
							...
							<table class="table table-condensed js-table-checkable table-hover js-dataTable-full">
								<thead>
									<tr>
										...
										<th class="hidden-xs">NIP / Wali Kelas</th>
								...
								<tbody>
									...
									$no=1;
										foreach($wali_kelas as $data)
										 ...
										 $jumlah = $this->kelas_wali_model->getJumlahMurid("tb_kelas","thn_ajaran_id =".$data->thn_ajaran_id,"tingkat_id=".$data->tingkat_id);
									...
									<tr>
										...
										<td class="text-center">
											<label class="css-input css-checkbox css-checkbox-success">
												<input type="checkbox" value="<?php echo $data->wali_kelas_id;?>" id="row_<?php echo $no;?>" name="kelas_wali[]"><span></span>
											</label>
										</td>
										<td class=""><?php echo $data->tahun;?></td>
										<td class="hidden-xs"><span data-toggle="popover" data-placement="top" title="<?php foreach($jumlah->result() as $j){echo $j->murid_id;} ?>"><?php echo $data->nama_tingkat. "</span> <label class='label label-default'>".count($jumlah->result())." murid</label>";?></td>
											
										<td class="hidden-xs"><span class="label <?php echo $label;?>"><?php echo $data->tingkat;?></span></td>
										<td class="hidden-xs"><?php echo "<span class='text-muted'>".$data->guru_nip."</span> ". anchor("guru/data_guru/$data->guru_id","$data->guru_nama","target='blank'");?></span></td>
										
										<td class="text-center">
											<div class="btn-group">
												<a href="kelas/edit/<?php echo $data->wali_kelas_id;?>" class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="Edit Wali Kelas"><i class="fa fa-pencil"></i></a>
												<a href="javascript:;" data-id="<?php echo $data->wali_kelas_id;?>" data-toggle="modal" data-target="#modal-konfirmasi" class="btn btn-xs btn-danger" type="button" title="Hapus kelas"><i class="fa fa-times"></i></a>
											</div>
										</td>
									</tr>
											<?php  
